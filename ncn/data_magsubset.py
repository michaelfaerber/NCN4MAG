...
def get_datasets(path_to_data: PathOrStr, 
    ...
    path_to_mag = os.path.join(path_to_data, "mag_data.csv")
    path_to_train = os.path.join(path_to_data, "mag_subset2_train.csv")
    path_to_valid = os.path.join(path_to_data, "mag_subset2_valid.csv")
    path_to_test = os.path.join(path_to_data, "mag_test.csv")
    ...
    dataset = TabularDataset('/home/maria/input/mag_subset2_data.csv', "CSV", [("context", CNTXT), ("authors_citing", AUT), ("title_cited", TTL), ("authors_cited", AUT)], skip_header=True)

    train, valid, test = TabularDataset.splits(path='/home/maria/input', train='mag_subset2_train.csv',
                                    validation='mag_subset2_valid.csv', test='mag_test.csv', 
                                    format='csv',
                                    fields=[("context", CNTXT), ("authors_citing", AUT), ("title_cited", TTL), ("authors_cited", AUT)],
                                    skip_header=True)
            
    """
    train = TabularDataset(path_to_train, "CSV", 
                       [("context", CNTXT), ("authors_citing", AUT), ("title_cited", TTL), ("authors_cited", AUT)],
                       skip_header=True)
    valid = TabularDataset(path_to_valid, "CSV", 
                       [("context", CNTXT), ("authors_citing", AUT), ("title_cited", TTL), ("authors_cited", AUT)],
                       skip_header=True)
    test = TabularDataset(path_to_test, "CSV", 
                       [("context", CNTXT), ("authors_citing", AUT), ("title_cited", TTL), ("authors_cited", AUT)],
                       skip_header=True)
    """
...
def get_bucketized_iterators(path_to_data: PathOrStr, batch_size: int = 16,
    ...
    """
    train_iterator = BucketIterator(data.train,
                                    batch_size = batch_size,
                                    sort_within_batch = True,
                                    sort_key = lambda x : len(x.title_cited))
    valid_iterator = BucketIterator(data.valid,
                                    batch_size = batch_size,
                                    sort_within_batch = True,
                                    sort_key = lambda x : len(x.title_cited))
    test_iterator = BucketIterator(data.test,
                                    batch_size = batch_size,
                                    sort_within_batch = True,
                                    sort_key = lambda x : len(x.title_cited))
    """
